    ...
    try {
      ...
      try {
        ...
        if (!Array.isArray(parsedData)) {
          throw new Error('Data must be an array of colleges');
        }
      } catch (err) {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to import colleges');
      }

      setResults(data);
      toast.success(`Successfully processed ${data.results.length} colleges`);
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8">
      <div className="px-4 sm:px-0">
        <h1 className="text-2xl font-semibold text-gray-900">Import Colleges</h1>
        <p className="mt-1 text-sm text-gray-600">
          Import multiple colleges at once using JSON format.
        </p>
      </div>
      ...
      >
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-gray-500">
            Enter JSON array of college objects or upload a JSON file.
          </p>
        </div>
